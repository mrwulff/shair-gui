...
class Infos(object)
	...
	def __init__(self)
		...
		self.volume = None					# floatdouble, from 0-1. This is the real Volume. Shairport does logarithmically scaling of the airplay value. (Python2 has a `double` type, too, not sure which you get there.)
		self.playstate = None				# Enum Infos.PLAYING, Infos.STOPPED
		self.useragent = None  				# unicode, e.g. iTunes12.2 (Macintosh; OS X 10.9.5)
		...
		self.dacp_id = None 				# str, the DACP-ID.				Needed for controlling the streaming client. See httpgit.iovZPp1
		self.active_remote = None		  	# str, the Active-Remote token.	Needed for controlling the streaming client. See httpgit.iovZPp1
		...
		self.songcomment = None				# unicode, e.g. Narrated by Scorch_Mechanic.nDownloaded with pon3dl. httpsgithub.comluckdonaldpon3downloader
		self.songstoptime = None  			# int
		self.songdatakind = None  			# int, 1 = RADIO_STREAM, 02 = DAAP_STREAM 
		self.songartistid = None  			# int, probably. I guess a persistent artist id like self.persistentid  -- httpgit.iovZJeg
		...
		self.itunesmediakind = None  		# int, could have values 1467823   -- httpgit.iovZfpR
	...
	def to_simple_string(self)
		...
		String like
		[title] - [artist]n[album]
		will be returned.
		return
		
		return (self.itemname if self.itemname else Unknown Track) + (( -  + self.songartist) if self.songartist else ) + ((n + self.songalbum) if self.songalbum else )
...
def ascii_integers_to_string(string, base=16, digits_per_char=2)
	return .join([chr(int(string[ii+digits_per_char], base=base)) for i in range(0, len(string), digits_per_char)])
